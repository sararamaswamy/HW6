...
class Student():
    def __init__(self, name, years_at_umich=1, programs_written=0):
        ...
        ## add a method called _write programs which accept one optional paramater with a defualt value of 1, and adds the input integer to the self.num_programs instance variable
    ...
    def write_programs(self, optx = 1):
    	self.num_programs = optx + self.num_programs
...
def personal_map(funcobj, a_list):
	# new_list = map(functionobj, a_list)
	new_list = map(funcobj, a_list)
...
## input type is list of FILENAMES
## yield each line in each of the files with that name
...
## input type generator (is file lines, like from above)
## return generator object, which contains ALL lines it accepted with length longer than 40 chars

def len_check(file_lines):
    return(line for line in file_lines if len(line) > 40)
